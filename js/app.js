...
function enterApp(name,{listen=true}={}){
  showScreen('app');
  const hu=document.getElementById('header-user'); if(hu) hu.textContent='Můj Profil';
...
function renderPlayerDashboard(request=accessRequest){
  ...
  const chart=points.length?`<div class="player-line-chart" aria-label="Kumulace pokut v čase"><svg viewBox="0 0 100 44" preserveAspectRatio="none"><polyline points="${points.map((point,index)=>`${points.length===1?50:(index/(points.length-1)*100).toFixed(1)},${(42-Math.abs(point.value)/maxAbs*36).toFixed(1)}`).join(' ')}" fill="none" stroke="#2563c7" stroke-width="1.8" vector-effect="non-scaling-stroke"/><line x1="0" y1="42" x2="100" y2="42" stroke="#cbd5e1" stroke-width=".6"/></svg><small>Začátek sezóny <span>aktuální vývoj</span></small></div>`:'<div class="player-chart-empty">V této sezóně zatím nemáš žádný záznam.</div>';
  ...
  const publicDebtors=Object.entries(teamSeasonFines.reduce((map,f)=>{const row=map[f.player]||(map[f.player]={total:0,reasons:{}});row.total+=Number(f.amount||0);const key=displayReason(f.reason);row.reasons[key]=(row.reasons[key]||0)+Number(f.amount||0);return map;},{})).sort((a,b)=>b[1].total-a[1].total).slice(0,5);
  const publicMax=Math.max(1,...publicDebtors.map(item=>item[1].total));
  const publicReasons=[...new Set(publicDebtors.flatMap(([,row])=>Object.keys(row.reasons)))];
  const publicPalette=['#123f91','#2563c7','#ef3340','#f59e0b','#16a34a','#7c3aed'];
  const stackedDebtors=publicDebtors.map(([name,row])=>{const segments=publicReasons.map((reason,index)=>{const amount=row.reasons[reason]||0;return amount?`<i class="player-stack-segment" style="width:${Math.max(0,amount/publicMax*100)}%;background:${publicPalette[index%publicPalette.length]}" title="${esc(reason)}: ${amount} CZK"></i>`:''}).join('');return `<div class="player-stacked-row"><span title="${esc(name)}">${esc(name)}</span><div class="player-stacked-bar">${segments}</div><strong>${row.total} ${CONFIG.CURRENCY}</strong></div>`;}).join('');
  ...
  const proposalHtml=proposals.length?`<div class="player-proposals"><div class="player-cumulative-title">Moje návrhy ke kontrole</div>${proposals.map(p=>{const status=p.status==='approved'?'Schváleno':p.status==='rejected'?'Zamítnuto':'Čeká na schválení';const submitted=p.createdAt?new Date(p.createdAt).toLocaleString('cs-CZ'):'';return `<div class="player-proposal-status status-${p.status}"><span>${esc(p.reason||UNKNOWN_REASON)} · ${p.amount} ${CONFIG.CURRENCY}</span><b>${status}</b><small>Odesláno: ${esc(submitted)}${p.rejectionReason?` · Důvod: ${esc(p.rejectionReason)}`:''}</small></div>`;}).join('')}</div>`:'<div class="player-proposals player-proposals-empty">Zatím jsi neposlal žádný návrh.</div>';
  if(content) content.innerHTML=`<div class="player-season-meta"><b>${esc(player||'Můj Profil')}</b><span>Sezóna ${esc(selectedSeason)}</span><span>Aktuální dlužná částka: <strong>${total} ${CONFIG.CURRENCY}</strong></span><span><strong>${rank>0?`${rank}. místo z ${rankData.length} registrovaných hráčů`:'Bez pořadí'}</strong> · Aktuálně evidováno: ${current.length} přestupků</span></div><div class="player-season-heading"><strong>${esc(selectedSeason)}</strong><span>${historical?'Historická sezóna · pouze pro čtení':'Aktuální sezóna'}</span></div><div class="player-score"><strong>${total} ${CONFIG.CURRENCY}</strong><span><b>${rank>0?`${rank}. místo z ${rankData.length}`:'Bez pořadí'}</b> · ${current.length} ${current.length===1?'záznam':'záznamů'}</span></div><div class="player-season-timeline"><div class="player-season-track"><i style="left:${marker}%"></i></div><div><span>15. 7. ${String((activeSeason||seasonForDate()).year).slice(-2)}</span><span>${now>=timelineStart&&now<=timelineEnd?'dnes · ':''}15. 6. ${String((activeSeason||seasonForDate()).year+1).slice(-2)}</span></div></div><div class="player-cumulative"><div class="player-cumulative-title">Vývoj pokut v sezóně</div>${chart}</div><div class="player-fine-breakdown">${current.length?current.map(f=>`<span>${esc(displayReason(f.reason))} <b>${f.amount} ${CONFIG.CURRENCY}</b></span>`).join(''):'<span>V této sezóně zatím nemáš žádné pokuty.</span>'}</div><div class="player-public-stats"><div><div class="player-cumulative-title">Fond sezóny · ${esc(selectedSeason)}</div><strong>${allTotal.toLocaleString('cs-CZ')} ${CONFIG.CURRENCY}</strong><small>${teamSeasonFines.length} záznamů · cíl ${SEASON_BUDGET.toLocaleString('cs-CZ')} ${CONFIG.CURRENCY}</small><div class="player-fund-track"><b style="width:${Math.min(100,Math.round(allTotal/SEASON_BUDGET*100))}%"></b></div></div><div><div class="player-cumulative-title">Největší hříšníci</div>${stackedDebtors||'<small>Zatím bez záznamů.</small>'}</div></div>${proposalHtml}`;
...
window.submitPlayerProposal=async function(){
  ...
  const proposal={id,uid:currentUser.uid,player:accessRequest.linkedPlayerName,reason,amount:signedAmount,season:seasonKey(activeSeason||seasonForDate()),status:'pending',createdAt:new Date().toISOString()};
  fineProposals=[proposal,...fineProposals.filter(item=>item.id!==id)]; renderPlayerDashboard(accessRequest);
  try{await setDoc(doc(db,'fineProposals',id),proposal);document.getElementById('player-proposal-amount').value='';showToast('Návrh pokuty byl odeslán ke schválení.');}
  catch(error){fineProposals=fineProposals.filter(item=>item.id!==id);renderPlayerDashboard(accessRequest);showToast('Návrh se nepodařilo uložit. Zkus to prosím znovu.');console.error('Fine proposal:',error);}
};
...
window.updateAccessSelectStyle=function(select){
  if(!select) return;
  select.classList.remove('access-status-approved','access-status-pending','access-status-rejected','access-role-admin','access-role-cashier','access-role-viewer');
  select.classList.add(select.id.startsWith('user-status-')?`access-status-${select.value}`:`access-role-${select.value}`);
};
...
function buildReviewQueue(transcript){
  ...
  if(unknown.length) setTimeout(()=>alert(`Hráč ${unknown.join(', ')} není v databázi. Přidej ho nejdřív v sekci Soupiska.`),0);
}
...
function renderReviewQueue(){
  ...
  document.getElementById('review-list').innerHTML=reviewQueue.map((r,i)=>{
    ...
    const playerCandidates=(r.candidates||[]).filter(c=>Number(c.score)>=0.5);
    const candidateHint=r.needsPlayer&&playerCandidates.length
      ?`<div class="review-warning review-player-hint"><strong>Nejbližší shoda:</strong>${playerCandidates.map(c=>`<button type="button" class="review-suggestion review-player-suggestion" onclick="updateReview(${i},'resolvedPlayer',${JSON.stringify(c.player).replace(/"/g,'&quot;')})">${esc(c.player)} · ${Math.round(c.score*100)} %</button>`).join('')}</div>`:'';
    const unknownPlayer=r.needsPlayer&&!playerCandidates.length?`<div class="review-warning review-new-player"><strong>Hráč „${esc(voiceDisplayName(r.rawName))}“ není v databázi.</strong><span>Přidej ho nejdřív v sekci Soupiska.</span></div>`:'';
...
// ─── DASHBOARD ───────────────────────────────────────────────────
const SEASON_BUDGET = 20000; // CZK target per season
